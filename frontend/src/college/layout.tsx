...
import {
  ...
  Building2, AlertTriangle, GraduationCap, LogOut, type LucideIcon,
} from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import { getMockCurrentUser } from './utils/auth';
import { CollegeRole, type CollegeRoleType } from './types';
...
const { COUNSELOR, DEAN, ADMIN } = CollegeRole;

const allNavItems: LocalNavItem[] = [
  { title: '管理驾驶舱', href: '/college/dashboard', icon: LayoutDashboard, roles: [COUNSELOR, DEAN, ADMIN] },
  { title: '教学教务管理', href: '/college/teaching', icon: BookOpen, roles: [COUNSELOR, DEAN, ADMIN] },
  { title: '实习监管', href: '/college/employment', icon: Briefcase, roles: [COUNSELOR, DEAN, ADMIN] },
  { title: '企业CRM', href: '/college/crm', icon: Building2, roles: [DEAN, ADMIN] },
  { title: '预警指挥室', href: '/college/warning', icon: AlertTriangle, roles: [COUNSELOR, DEAN, ADMIN] },
];

const roleLabelMap: Record<CollegeRoleType, { label: string; className: string }> = {
  [COUNSELOR]: { label: '辅导员', className: 'border-transparent bg-green-500 text-white' },
  [DEAN]: { label: '系主任', className: 'border-transparent bg-purple-500 text-white' },
  [ADMIN]: { label: '教务', className: 'border-transparent bg-orange-500 text-white' },
};
    ...
    <div className="flex h-screen bg-background overflow-hidden">
      <aside className="border-r bg-card hidden md:flex flex-col w-16 lg:w-64">
        <div className="h-16 flex items-center justify-center lg:justify-start lg:px-6 border-b">
          <div className="h-8 w-8 bg-green-600 rounded-lg flex items-center justify-center shrink-0">
            <GraduationCap className="h-5 w-5 text-white" />
          </div>
          <span className="font-bold text-lg tracking-tight ml-2 hidden lg:block">智途·高校</span>
        </div>
